...
{
    public class StudentFavoritsCourseRipository:IStudentFavoritsCourseRipository
    {
        private readonly AllamehPrroject _context;

        public StudentFavoritsCourseRipository(AllamehPrroject context)
        {
            _context = context;
        }

        public async Task<AddStatusVm> Add(StudentFavoriteCourses studentCourses)
        {
            try
            {
                ...
                _context.Set<StudentFavoriteCourses>().Add(studentCourses);
        ...
        public async Task<UpdateStatus> Update(StudentFavoriteCourses studentCourses)
        {
            try
            {
                var existingRecord = await _context.Set<StudentFavoriteCourses>()
                ...
                _context.Set<StudentFavoriteCourses>().Update(existingRecord);
                await _context.SaveChangesAsync();
        ...
        {
            var result = await _context.Set<StudentFavoriteCourses>()
        ...
        {
            return await _context.Set<StudentFavoriteCourses>()
        ...
        public async Task<UpdateStatus> Delete(Guid courseId, Guid userId)
        {
            try
            {
                var studentCourse = await _context.Set<StudentFavoriteCourses>()
                ...
                _context.Set<StudentFavoriteCourses>().Remove(studentCourse);
                await _context.SaveChangesAsync();
